...
warn_optional_path() {
  ...
  local reason="$2"
  if [ ! -e "$path" ]; then
    warn "Optional path is missing: $path"
    warn "$reason"
  fi
}
...
verify_bundle_layout() {
  ...
  warn_optional_path "$CLIPROXY_SRC/bin" "CLIProxyAPI will be built from source if a matching bundled binary is not present."
  warn_optional_path "$OPENCLAW_SRC/dist" "OpenClaw dist will be built from source during installation if it is not present."
}
...
normalize_linux_arch() {
  ...
  case "$raw_arch" in
    ...
    i386|i686) echo "386" ;;
    armv6l|armv7l|armhf) echo "armv6l" ;;
...
install_google_chrome() {
  local arch
  local key_file="$CACHE_DIR/google-chrome-signing-key.pub"
...
install_go_toolchain() {
  ...
  case "$go_arch" in
    ...
    *)
      fail "No official Go binary mapping for architecture: $raw_arch"
      ;;
  esac
  ...
  archive_path="$CACHE_DIR/go${requested_version}.linux-${go_arch}.tar.gz"
  echo "Installing Go $requested_version for $raw_arch from official distribution..."
  retry_command 3 5 curl -fsSL "https://go.dev/dl/go${requested_version}.linux-${go_arch}.tar.gz" -o "$archive_path"
  rm -rf /usr/local/go
...
install_cliproxy_binary() {
  ...
  bundled_binary="/opt/cli-proxy-api/bin/cli-proxy-api-linux-$cli_arch"

  if [ -f "$bundled_binary" ]; then
    echo "Using bundled CLIProxyAPI binary for $cli_arch_raw ($cli_arch)..."
    install -m 0755 "$bundled_binary" /usr/local/bin/cli-proxy-api
    return 0
  fi

  go_version="$(resolve_cliproxy_go_version /opt/cli-proxy-api)"
  [ -n "${go_version:-}" ] || fail "Unable to determine CLIProxyAPI Go version from go.mod"

  echo "No bundled CLIProxyAPI binary for $cli_arch_raw ($cli_arch); building from source with Go $go_version..."
  install_go_toolchain "$go_version" "$cli_arch_raw"
...
install_openclaw_runtime() {
  ...
  if [ ! -d /opt/openclaw-zero-token/dist ]; then
    echo "OpenClaw dist is missing; installing full dependencies and building from source..."
    if ! retry_command 2 5 pnpm install --frozen-lockfile; then
      echo "Frozen install failed, retrying without --frozen-lockfile..."
      retry_command 2 5 pnpm install
    fi
    retry_command 2 5 pnpm build
    [ -d /opt/openclaw-zero-token/dist ] || fail "OpenClaw build finished but dist directory is still missing."
    pnpm prune --prod || warn "pnpm prune --prod failed; continuing with installed dependencies."
  else
    if ! retry_command 2 5 pnpm install --prod --frozen-lockfile; then
      echo "Frozen production install failed, retrying without --frozen-lockfile..."
...
main() {
  ...
  local node_major
  local openclaw_pnpm_version
  local cliproxy_mgmt_secret_hash
  local host
  local origin
  ...
  if ! command_exists apt-get; then
    fail "This script currently targets Debian/Ubuntu/Armbian style systems."
  fi

  if ! command_exists systemctl; then
    fail "systemctl is required. Use a Debian/Ubuntu/Armbian host with systemd."
  fi
  ...
  if [ "$need_node_install" -eq 1 ]; then
    ...
    install -d -m 0755 /etc/apt/keyrings
    retry_command 3 5 curl -fsSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key -o "$CACHE_DIR/nodesource.gpg.key"
    gpg --dearmor < "$CACHE_DIR/nodesource.gpg.key" > /etc/apt/keyrings/nodesource.gpg
...
EOF
  ...
  echo "LAN/private IP: $N1_LAN_IP"
  if [ -n "$PUBLIC_ACCESS_HOST" ]; then
    echo "Public host: $PUBLIC_ACCESS_HOST"
  fi
  echo
  echo "Access details were written to:"
  echo "  /opt/cli-proxy-api/ACCESS.txt"
  echo "  /opt/openclaw-zero-token/ACCESS.txt"
  echo
  echo "Read them with:"
  echo "  sudo cat /opt/cli-proxy-api/ACCESS.txt"
  echo "  sudo cat /opt/openclaw-zero-token/ACCESS.txt"
  echo
