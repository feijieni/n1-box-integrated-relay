...
async function syncModelsProvidersToConfigLight(providerIds: string[]): Promise<void> {
  const currentDir = path.dirname(fileURLToPath(import.meta.url));
  const scriptPath = path.resolve(currentDir, "../../scripts/sync-webauth-config.mjs");
  const { stdout, stderr } = await execFileAsync(process.execPath, [scriptPath, ...providerIds], {
    env: process.env,
  });
...
export async function runOnboardWebAuth(): Promise<void> {
  ...
  if (selectedProviders.length > 0) {
    await syncModelsProvidersToConfigLight(selectedProviders.map((provider) => provider.id));
  }
