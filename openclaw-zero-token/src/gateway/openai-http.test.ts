    ...
    try {
      {
        ...
        const stopChoice = jsonChunks
          .flatMap((c) => (c.choices as Array<Record<string, unknown>> | undefined) ?? [])
          .find((choice) => choice.finish_reason === "stop");
        expect(stopChoice).toBeTruthy();
      }
...

      {
        ...
        agentCommand.mockImplementationOnce(
          ((opts: unknown) =>
            new Promise((resolve) => {
              const emitText = "Let me pull market data first.";
              const finalText =
        ...
        const delayedRes = await postChatCompletions(port, {
          stream: true,
          model: "openclaw",
          messages: [{ role: "user", content: "hi" }],
        });
        expect(delayedRes.status).toBe(200);
        const delayedText = await delayedRes.text();
        const delayedData = parseSseDataLines(delayedText);
        expect(delayedData[delayedData.length - 1]).toBe("[DONE]");

        const delayedChunks = delayedData
          .filter((d) => d !== "[DONE]")
          .map((d) => JSON.parse(d) as Record<string, unknown>);
        const delayedContent = delayedChunks
          .flatMap((c) => (c.choices as Array<Record<string, unknown>> | undefined) ?? [])
          .map((choice) => (choice.delta as Record<string, unknown> | undefined)?.content)
          .filter((v): v is string => typeof v === "string")
          .join("");
        expect(delayedContent).toBe(
          "Let me pull market data first. The full answer arrives after the tool work.",
        );
        const delayedStopChoice = delayedChunks
          .flatMap((c) => (c.choices as Array<Record<string, unknown>> | undefined) ?? [])
          .find((choice) => choice.finish_reason === "stop");
        expect(delayedStopChoice).toBeTruthy();
      }
