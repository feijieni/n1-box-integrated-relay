  ...
  it("streams OpenResponses SSE events", async () => {
    ...
    try {
      ...
      agentCommand.mockImplementationOnce(
        ((opts: unknown) =>
          new Promise((resolve) => {
            const runId = ((opts as { runId?: string } | undefined)?.runId ?? "").trim();
            const firstText = "Let me pull market data first.";
            const finalText =
              "Let me pull market data first. The full answer arrives after the tool work.";
            ...
            setTimeout(() => resolve({ payloads: [{ text: finalText }] }), 10);
          })) as never,
      );

      const resEarlyLifecycle = await postResponses(port, {
        stream: true,
        model: "openclaw",
        input: "hi",
      });
      expect(resEarlyLifecycle.status).toBe(200);

      const earlyLifecycleText = await resEarlyLifecycle.text();
      const earlyLifecycleEvents = parseSseEvents(earlyLifecycleText);
      const earlyLifecycleDeltas = earlyLifecycleEvents
        .filter((e) => e.event === "response.output_text.delta")
        .map((e) => {
          const parsed = JSON.parse(e.data) as { delta?: string };
          return parsed.delta ?? "";
        ...
        .join("");
      expect(earlyLifecycleDeltas).toBe(
        "Let me pull market data first. The full answer arrives after the tool work.",
      );

      const earlyLifecycleDone = earlyLifecycleEvents.find(
        (event) => event.event === "response.output_text.done",
      );
      expect(earlyLifecycleDone).toBeTruthy();
      expect((JSON.parse(earlyLifecycleDone?.data ?? "{}") as { text?: string }).text).toBe(
        "Let me pull market data first. The full answer arrives after the tool work.",
      );
      expect(earlyLifecycleEvents.some((event) => event.data === "[DONE]")).toBe(true);
