...
import {
  resolveAssistantMergedText,
  resolveAssistantStreamTextUpdate,
  resolveAssistantTextAppend,
...
export async function handleOpenResponsesHttpRequest(
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "assistant") {
      const update = resolveAssistantStreamTextUpdate({ previousText: accumulatedText, evt });
  ...
  void (async () => {
    try {
      ...
      const { stopReason, pendingToolCalls } = resolveStopReasonAndPendingToolCalls(meta);
      const payloadText = resolveResponsePayloadText(resultAny);

      if (!closed) {
        let mergedText = resolveAssistantMergedText({
          previousText: accumulatedText,
          nextText: payloadText,
        });
        let deltaText = resolveAssistantTextAppend({
          previousText: accumulatedText,
          nextText: mergedText,
        });
        const fallbackText = mergedText || payloadText || "No response from OpenClaw.";

        if (!sawAssistantDelta && !deltaText) {
        ...
        if (finalizeRequested) {
          finalizeRequested.text = accumulatedText || fallbackText;
        }
      }
      ...
      if (
        ...
        const finalText = accumulatedText || payloadText || "No response from OpenClaw.";
