...
export async function handleOpenAiHttpRequest(
  ...
  const unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "assistant") {
      const update = resolveAssistantStreamTextUpdate({ previousText: streamedText, evt });
  ...
  void (async () => {
    try {
      ...
      const content = resolveAgentResponseText(result);
      const finalText = resolveAssistantMergedText({
        previousText: streamedText,
        nextText: content,
      });
      const finalAppend = resolveAssistantTextAppend({
        previousText: streamedText,
        nextText: finalText,
      });
      const finalChunkContent = sawAssistantDelta ? finalAppend : finalText || content;
      ...
      if ((!sawAssistantDelta || finalAppend) && finalChunkContent) {
        sawAssistantDelta = true;
